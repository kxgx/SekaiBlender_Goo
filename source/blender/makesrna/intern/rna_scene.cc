...
static void rna_def_scene_eevee(BlenderRNA *brna)
{
  ...
  static const EnumPropertyItem eevee_fsr_quality_items[] = {
      {SCE_EEVEE_FSR_QUALITY_ULTRA,
       "ULTRA_QUALITY",
       0,
       "Ultra Quality",
       "Render at 77% internal resolution"},
      {SCE_EEVEE_FSR_QUALITY_QUALITY,
       "QUALITY",
       0,
       "Quality",
       "Render at 67% internal resolution"},
      {SCE_EEVEE_FSR_QUALITY_BALANCED,
       "BALANCED",
       0,
       "Balanced",
       "Render at 59% internal resolution"},
      {SCE_EEVEE_FSR_QUALITY_PERFORMANCE,
       "PERFORMANCE",
       0,
       "Performance",
       "Render at 50% internal resolution"},
      {0, nullptr, 0, nullptr, nullptr},
  };
  ...
  prop = RNA_def_property(srna, "use_fsr_viewport", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", SCE_EEVEE_FSR_VIEWPORT);
  RNA_def_property_ui_text(
      prop, "FSR Viewport", "Render the EEVEE viewport at half resolution and upscale using FSR");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "use_fsr_render", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", SCE_EEVEE_FSR_RENDER);
  RNA_def_property_ui_text(
      prop, "FSR Render", "Render EEVEE images at half resolution and upscale using FSR");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "fsr_sharpness", PROP_FLOAT, PROP_FACTOR);
  RNA_def_property_ui_text(prop, "Sharpness", "Strength of FSR edge-adaptive sharpening");
  RNA_def_property_range(prop, 0.0f, 1.0f);
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "fsr_quality", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "fsr_quality");
  RNA_def_property_enum_items(prop, eevee_fsr_quality_items);
  RNA_def_property_ui_text(prop, "Quality", "Internal render resolution used by FSR 1");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);
  ...
  RNA_def_property_ui_text(prop,
                           "Shadow Pool Size",
                           "Initial size of the shadow pool; it automatically grows when needed, "
                           "up to the 2 GB packed-page limit");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

...
/** Return true when this MMD approximation belongs to a native MMD IK chain.
 *
...
static bool is_native_mmd_ik_approx(Object &ob,
                                     ...
                                     bConstraint &con)
{
  if (std::strcmp(con.name, "MMD_IK_Approx") != 0 || ob.data == nullptr) {
    return false;
  }
  ...
  if (!use_v8 && !use_legacy && ob.adt != nullptr && ob.adt->action != nullptr) {
    return false;
  }
  if (!use_legacy) {
  ...
  for (int i = 0; i < ik_bones->len; i++) {
    ...
    for (int link_index = 0; link_index < links->len; link_index++) {
      IDProperty *link = IDP_GetIndexArray(links, link_index);
...
static bool constraint_valid(Object &ob, bPoseChannel &pchan_tip, bConstraint *con)
{
  ...
  /* A native MMD CCD chain is exclusive: do not let its Blender/iTaSC
   * approximation enter the solver tree. */
  if (is_native_mmd_ik_approx(ob, pchan_tip, *con)) {
    return false;
  }
  /* An IK constraint with zero influence is inactive. This check is
   * required here because KINEMATIC constraints are consumed by iTaSC
   * directly rather than by BKE_constraints_solve(), whose generic path
   * performs this check for us. */
...
static int initialize_scene(Object *ob, bPoseChannel *pchan_tip)
{
  ...
  for (bConstraint &con : pchan_tip->constraints) {
    if (con.type == CONSTRAINT_TYPE_KINEMATIC) {
      if (constraint_valid(*ob, *pchan_tip, &con)) {
                          ...
                          float frtime)
{
  ...
  for (i = ikscene->targets.size(); i > 0; i--) {
    ...
    if (!(iktarget->blenderConstraint->flag & CONSTRAINT_OFF) &&
        iktarget->blenderConstraint->enforce != 0.0f)
    {
      break;
    }
  }
  ...
  for (i = ikscene->targets.size(); i > 0; i--) {
    IK_Target *iktarget = ikscene->targets[i - 1];
    if (!(iktarget->blenderConstraint->flag & CONSTRAINT_OFF) &&
        iktarget->blenderConstraint->enforce != 0.0f && iktarget->constraint)
    {
      uint nvalues;

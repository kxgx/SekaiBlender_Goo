 ...
 *
 * Goo-engine 移植（C 包）：Set Depth 节点变体。
 * 与 surf_deferred 相同，但去掉 early_fragment_tests，允许材质 GLSL（node_set_depth）
 * 写 gl_FragDepth 覆盖像素深度。仅对含 ShaderNodeSetDepth 的材质管线启用。
 *
 * 注意：本文件必须自包含，不能 include eevee_surf_deferred.bsl.hh——
 * 其 [[fragment]] [[early_fragment_tests]] 入口经 lower_entry_points 会把
 * EARLY_FRAGMENT_TEST(true) 泄漏进最终 GLSL，驱动将丢弃 gl_FragDepth 写入。
 */
#pragma once
...
/* 无 early_fragment_tests：Set Depth 节点（node_set_depth）通过 gl_FragDepth 覆盖深度。 */
[[fragment]]
void surf_deferred_depth([[resource_table]] PipelineConstants &pipe,
                   [[resource_table]] SurfaceDeferredDepth &srt,
                   ...
                   [[out]] DeferredFragOutDepth &frag_out,

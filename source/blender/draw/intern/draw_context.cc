...
void DRW_draw_view(const bContext *C)
{
  ...
  /* FSR Diagnostics: capture viewport render texture every N frames. */
  {
    static int frame_counter = 0;
    frame_counter++;
    gpu::Texture *color_tex = viewport ? GPU_viewport_color_texture(viewport, 0) : nullptr;
    if (color_tex != nullptr) {
      draw::FSR_diagnostics_capture(color_tex,
                              nullptr,
                              "viewport",
                              GPU_texture_width(color_tex),
                              GPU_texture_height(color_tex),
                              frame_counter);
    }
  }
}
                                    ...
                                    GPUViewport *viewport)
{
  ...
  /* FSR Diagnostics: capture render output texture. */
  {
    static int render_frame_counter = 0;
    render_frame_counter++;
    gpu::Texture *color_tex = GPU_viewport_color_texture(render_viewport, 0);
    if (color_tex != nullptr) {
      draw::FSR_diagnostics_capture(color_tex,
                                    nullptr,
                                    "render",
    ...
    std::function<void(RenderResult *)> store_metadata_cb)
{
  ...
  /* FSR Diagnostics: ensure auto-init runs at least once during startup. */
  {
    static bool fsr_diag_inited = false;
    if (!fsr_diag_inited) {
      fsr_diag_inited = true;
      draw::FSR_diagnostics_init();
    }
  }
